...
static void json_escape_to_buf(const char *s, char *out, size_t out_sz) {
    if (!out || out_sz == 0) return;
    out[0] = '\0';
    if (!s) return;

    size_t j = 0;
    for (const unsigned char *p = (const unsigned char*)s; *p && j + 1 < out_sz; ++p) {
        unsigned char ch = *p;
        const char *rep = NULL;
        char tmp[8];
        tmp[0] = '\0';
...
static void handle_exec(supervisor_ctx_t *c, struct seccomp_notif *req) {
	...
	if (c->mode == MODE_DEV) {
		...
		// "commit" here means: we have decided to allow this exec and are about to CONTINUE it.
		emit_evt(c, "exec_commit", "\"abs\":\"%s\",\"path\":\"%s\",\"reason\":\"dev\"", abs, path);

		// Force comm/pid cache refresh on next event after exec changes identity.
		c->cache_tid = -1;

		// Breadcrumb: if you see this but no subsequent loader events, NOTIF_SEND likely failed.
		emit_evt(c, "exec_continue_send", "\"id\":%llu", (unsigned long long)req->id);

		int rc = respond_continue_chk(c->notify_fd, req->id);
		if (rc < 0) {
			emit_evt(c, "notif_send_err",
					 "\"op\":\"continue\",\"id\":%llu,\"errno\":%d",
					 (unsigned long long)req->id, -rc);
			hard_kill(c, "notif_send_failed");
		}
		return;
	}
